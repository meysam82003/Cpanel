...
final class ErrorGuidanceService
{
    ...
    public function for(string $code, string $language): array
    {
        ...
        $group = match (true) {
            ...
            str_contains($code, 'backup') => 'backup',
            str_contains($code, 'archive'), str_contains($code, 'zip_slip'), str_contains($code, 'zip_bomb') => 'archive',
    ...
    private function catalogue(): array
    {
        return [
            'fa' => [
                ...
                'backup' => ['title' => 'عملیات Backup یا بازیابی کامل نشد.', 'causes' => ['Artifact هنوز در حال ساخت است، منقضی شده یا checksum آن تغییر کرده است.', 'فضای امن موقت یا فضای حساب برای ساخت/Restore کافی نیست.', 'Job دیگری روی همین Backup فعال است یا Provider این نوع Restore را ارائه نمی‌کند.'], 'actions' => ['وضعیت و Progress را Refresh و فقط از Artifact با وضعیت completed استفاده کنید.', 'فضای Disk، دسترسی File/Database و وجود فایل Backup را بررسی کنید.', 'برای Full Account Restore با Provider یا مدیر WHM هماهنگ کنید و برای سایر انواع راهنمای Backup را باز کنید.']],
                'archive' => ['title' => 'عملیات Archive به‌صورت امن کامل نشد.', 'causes' => ['Archive دارای مسیر ناامن، link، ورودی رمزگذاری‌شده یا ساختار خراب است.', 'حجم بازشده، تعداد فایل‌ها یا نسبت فشرده‌سازی از Policy عبور کرده است.', 'فایل مقصد موجود است یا نتیجه cPanel پس از قطع ارتباط قطعی نیست.'], 'actions' => ['نام Archive، مقصد و Collision Policy را بررسی کنید.', 'در وضعیت Reconciliation فایل‌های مقصد را بررسی و سپس یک Job جدید بسازید.', 'برای BZIP2 از فعال بودن افزونه bz2 روی PHP هاست مطمئن شوید.']],
            ...
            'en' => [
                ...
                'backup' => ['title' => 'The backup or recovery operation did not complete.', 'causes' => ['The artifact is still being created, has expired, or its checksum changed.', 'Secure staging space or account disk space is insufficient for creation or restore.', 'Another job is using this backup, or the provider does not expose this restore type.'], 'actions' => ['Refresh status and progress, and use only an artifact marked completed.', 'Check disk space, File/Database access, and that the backup file still exists.', 'Contact the provider or WHM administrator for full-account restore; open Backup help for other types.']],
                'archive' => ['title' => 'The archive operation did not complete safely.', 'causes' => ['The archive contains an unsafe path, link, encrypted entry, or malformed structure.', 'Expanded size, file count, or compression ratio exceeded policy.', 'A destination exists or cPanel\'s outcome is uncertain after a connection failure.'], 'actions' => ['Review the archive name, destination, and collision policy.', 'For reconciliation state, inspect destination files and submit a new job only afterward.', 'For BZIP2, confirm that the hosting PHP build enables the bz2 extension.']],
