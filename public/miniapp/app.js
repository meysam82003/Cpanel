...
class ManagerApp {
  ...
  async uploadFiles(files, directory) {
    ...
    this.openForm('upload', `<div class="notice"><strong>${escapeHtml(t('info'))}</strong>${escapeHtml(t('dragUpload'))}</div><div class="field"><label>${escapeHtml(t('collision'))}</label><select name="collision"><option value="reject">${escapeHtml(t('reject'))}</option><option value="rename">${escapeHtml(t('autoRename'))}</option><option value="overwrite">${escapeHtml(t('overwrite'))}</option></select></div><div class="job"><div class="progress"><span id="upload-bar" style="width:0%"></span></div><span id="upload-text">${escapeHtml(t('uploadProgress', {percent: 0}))}</span></div>`, 'upload', async form => {
      const data = new FormData();
      files.slice(0, 20).forEach(file => data.append('files[]', file, file.name));
      data.append('directory', directory);
      data.append('collision', form.elements.collision.value);
  ...
  async pageSecurity() {
    ...
    this.content.innerHTML = `${this.pageHead('securityCenter')}<div class="notice danger"><strong>${escapeHtml(t('danger'))}</strong>${escapeHtml(t('warningToken'))}</div><div class="cards"><section class="card"><h2>${escapeHtml(t('connectedHosts'))}</h2><div class="list">${hosts || `<p class="muted">${escapeHtml(t('noHostTitle'))}</p>`}</div></section><section class="card"><h2>${escapeHtml(t('activeSessions'))}</h2><div class="list">${sessions || `<p class="muted">${escapeHtml(t('noRowsTitle'))}</p>`}</div></section><section class="card"><h2>${escapeHtml(t('alerts'))}</h2><div class="list">${alerts || `<p class="muted">${escapeHtml(t('noRowsTitle'))}</p>`}</div></section><section class="card"><h2>${escapeHtml(t('destructive'))}</h2><div class="list">${destructive || `<p class="muted">${escapeHtml(t('noRowsTitle'))}</p>`}</div></section></div>`;
  }
  ...
  async pageAdmin() {
    ...
    if (tab === 'dashboard') {
      ...
      body = `<div class="cards">${[['users', totals.users], ['connectedHosts', totals.hosts], ['apiRequests', totals.api_requests_today], ['errors', totals.errors_today], ['queue', totals.queued], ['alerts', totals.security_alerts]].map(([label, value]) => `<article class="card stat"><strong>${Number(value || 0)}</strong><span>${escapeHtml(t(label))}</span></article>`).join('')}</div><article class="card"><h2>${escapeHtml(t('errors'))}</h2><pre class="code">${escapeHtml(formatJson(result.recent_errors || []))}</pre></article>`;
    } else if (tab === 'users') {
  ...
  async handleAction(action, data, element = null) {
    const handlers = {
      ...
      'sql-execute': () => this.executeSql(), 'sql-explain': () => this.explainSql(), 'sql-format': () => this.formatSql(), 'sql-save-query': () => this.saveSqlQuery(), 'sql-clear': () => this.sqlEditor?.setValue('', -1), 'sql-load-history': () => this.sqlEditor?.setValue(this.sqlHistory[Number(data.index)]?.query || '', -1), 'sql-load-saved': () => this.sqlEditor?.setValue(this.savedQueries[Number(data.index)]?.query || '', -1), 'sql-delete-saved': () => this.deleteSavedQuery(Number(data.id)),
  ...
  async downloadRemote(path) { const result = await this.api.request(this.hostPath('/download-links'), {method: 'POST', body: {path}}); const anchor = document.createElement('a'); anchor.href = `/download/${encodeURIComponent(result.token)}`; anchor.rel = 'noopener'; anchor.download = result.filename || ''; document.body.appendChild(anchor); anchor.click(); anchor.remove(); }
  ...
  importDatabase(database = '') { this.openForm('importSql', `<div class="notice warning"><strong>${escapeHtml(t('warning'))}</strong>${escapeHtml(t('warningDatabase'))}</div><div class="field"><label>${escapeHtml(t('databaseName'))}</label><input name="database" value="${escapeAttr(database || this.params.database || '')}" required dir="ltr"></div><div class="field"><label>SQL / SQL.GZ / ZIP</label><input name="file" type="file" accept=".sql,.sql.gz,.gz,.zip" required></div><label class="check"><input name="backup_first" type="checkbox" checked>${escapeHtml(t('backupFirst'))}</label><div id="job-progress"></div>`, 'importSql', async form => { const file = form.elements.file.files[0]; if (!file) return; const db = form.elements.database.value, data = new FormData(); data.append('file', file, file.name); data.append('backup_first', form.elements.backup_first.checked ? '1' : '0'); $('#job-progress').innerHTML = this.progressHtml(0, t('uploadProgress', {percent: 0})); const result = await this.api.upload(this.hostPath(`/databases/${encodeURIComponent(db)}/imports`), data, percent => this.updateProgress(percent, t('uploadProgress', {percent}))); await this.pollJob(result.job_id, false); }); }
  ...
  async executeSql() { const sql = this.sqlEditor?.getValue() || '', database = this.params.database; const analysis = await this.api.request(this.hostPath(`/databases/${encodeURIComponent(database)}/sql/analyze`), {method: 'POST', body: {sql}}); const perform = async confirmation => { const result = await this.api.request(this.hostPath(`/databases/${encodeURIComponent(database)}/sql`), {method: 'POST', body: {sql, confirmation, backup_first: $('#sql-backup')?.checked || false, save_history: true}}); this.renderSqlResult(result.execution || result); this.toast(t('success')); this.haptic('success'); }; if (analysis.requires_confirmation) { const hash = await this.sha256(sql); return this.confirmOperation({warningKey: 'warningSql', action: 'sql.execute', target: hash, summary: `${analysis.type}\n${(analysis.reasons || []).join(', ')}\n\n${sql.slice(0, 2000)}`, preview: analysis, perform}); } return perform(null); }
  async explainSql() { const sql = this.sqlEditor?.getValue() || ''; const result = await this.api.request(this.hostPath(`/databases/${encodeURIComponent(this.params.database)}/sql/explain`), {method: 'POST', body: {sql}}); this.renderSqlResult(result); }
  ...
  changePhpIni() { const defaults = {memory_limit: '256M', max_execution_time: '30', upload_max_filesize: '20M', post_max_size: '24M'}; this.openForm('iniDirectives', `<div class="notice warning"><strong>${escapeHtml(t('warning'))}</strong>${escapeHtml(t('phpIniWarning'))}</div><div class="field"><label>${escapeHtml(t('scope'))}</label><select name="type"><option value="home">home</option><option value="domain">${escapeHtml(t('domain'))}</option></select></div><div class="field"><label>${escapeHtml(t('iniDirectives'))} (JSON)</label><textarea name="directives" rows="8" dir="ltr">${escapeHtml(formatJson(defaults))}</textarea></div>`, 'save', async form => { const type = form.elements.type.value, directives = this.parseJson(form.elements.directives.value, 'ini_directives'); this.closeDialog(); await this.confirmOperation({warningKey: 'warningFileDelete', action: 'php.ini', target: type, summary: `${type}\n${formatJson(directives)}`, preview: {directives: Object.keys(directives)}, perform: async confirmation => { await this.api.request(this.hostPath('/php/ini'), {method: 'PATCH', body: {type, directives, confirmation}}); await this.render(); }}); }); }
